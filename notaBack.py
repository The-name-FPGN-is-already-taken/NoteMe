    ...
    def __str__(self) -> str:
        ...
        for row in self.table.li:
            s += "{} {} {} {} {} {} {} {} {} {} \n".format(row.taskID, row.userID, row.taskType,
                                                           row.dateCreate, row.dateTarget, row.topic, row.description, row.day, row.star, row.finish)
        return s
    ...
    def addRecord(self, dateTarget: datetime, type: int = 0, topic: str = "No detail", descrption: str = "No descrption", day: int = -1, star=0, finish=0):
        ...
        try:
            ...
            if type == 0:
                ...
                self.writeTaskTable([lastIndex+1, self.userID, type, datetime.datetime.now().strftime(
                    "%Y-%m-%d %H:%M:%S"), dateTarget, topic, descrption, day, star, finish])
            elif type == 1:
                ...
                ).strftime("%Y-%m-%d %H:%M:%S"), -1, topic, descrption, day, star, finish])
            else:
                print("No type")
            self.refreshTable()
        except:
            ...
            for row in lines:
                if int(row.split(",")[0]) == int(obj.taskID):
                    f.write("{},{},{},{},{},{},{},{},{},{}\n".format(obj.taskID, obj.userID, obj.taskType,
                                                                     obj.dateCreate, obj.dateTarget, obj.topic, obj.description, obj.day, obj.star, obj.finish))
                else:
                    f.write(row)
        self.refreshTable()
